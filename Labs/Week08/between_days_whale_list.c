// Week 8 Lab
// Required to print whales inbetween two given dates
// 
// Dan Nguyen (z5206032)
// 14/09/2018
// 
...
void between_days_whales (struct pod *, struct date *, struct date *);
int check_start_date (struct date *, struct date *, struct date *);
int check_finish_date (struct date *, struct date *, struct date *);
...
void between_days_whales(struct pod *first_pod, struct date *start_day,
    ...
    while (current != NULL) {
        if (check_start_date(current->when, start_day, finish_day) == TRUE &&
            check_finish_date(current->when, start_day, finish_day) == TRUE) {
...
int check_start_date (struct date *current, struct date *start,
    struct date *finish) {
    
    if ((start->year < current->year) ||
        (start->year == current->year && start->month < current->month) ||
        (start->month == current->month && start->day <= current->day)) {
...
int check_finish_date (struct date *current, struct date *start,
    struct date *finish) {
    
    if ((finish->year > current->year) ||
        (finish->year == current->year && finish->month > current->month) ||
        (finish->month == current->month && finish->day >= current->day)) {
