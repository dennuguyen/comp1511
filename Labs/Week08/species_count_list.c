// Week 8 Lab
// Required to return number of pods and whales using linked lists.
// 
// Dan Nguyen (z5206032)
// 13/09/2018
// 
...
void species_count(char species[], struct pod *first_pod, int *n_pods,
    int *n_whales);
...
void species_count(char species[], struct pod *first_pod, int *n_pods,
    ...
    while (current != NULL) {
        if (strcmp(current->species, species) == 0) {
            n_pods_count++;
            n_whales_count += current->how_many;
        }
