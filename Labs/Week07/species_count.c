// Week 7 Lab
// Program that counts number of a particular species.
// 
// Dan Nguyen (z5206032)
// 2018/09/08
// 
...
void species_count(char species[], int n_sightings,
    struct pod sightings[n_sightings], int *n_pods, int *n_whales);
...
int main(int argc, char *argv[]) {
    ...
    int n_sightings = read_sightings_file(filename,
        MAX_SIGHTINGS, whale_sightings);
    ...
    int pod_count;
    int whale_count;
    species_count(species, n_sightings, whale_sightings,
        &pod_count, &whale_count);
    printf("%d %s pods containing %d whales in %s\n", pod_count,
        species, whale_count, filename);
...
void species_count(char species[], int n_sightings,
    ...
    int n_pods_count = 0;
    int n_whales_count = 0;
    
    for (int n = 0; n < n_sightings; n++) {
        if (strcmp(sightings[n].species, species) == 0) {
            n_pods_count++;
            n_whales_count += sightings[n].how_many;
        }
    }
    
    *n_pods = n_pods_count;
    *n_whales = n_whales_count;
}
...
int read_sightings_file(char filename[], int len, struct pod sightings[len]) {
    ...
    int n_sightings = 0;
    while (read_sighting(f, &sightings[n_sightings]) == 1 &&
        n_sightings < len) {
