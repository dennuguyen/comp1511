// Week 7 Lab
// Program that prints the number of orca pods.
// 
// Dan Nguyen (z5206032
// 2018/09/04
// 
...
int main(int argc, char *argv[]) {
    ...
    int n_sightings = read_sightings_file(argv[1],
...
int count_orca_sightings(int n_sightings, struct pod sightings[n_sightings]) {
    ...
    for (int n = 0; n < n_sightings; n++) {
        if (strcmp(sightings[n].species, "Orca") == 0) {
            n_orca_pods++;
        }
    }
...
int read_sightings_file(char filename[], int len, struct pod sightings[len]) {
    ...
    int n_sightings = 0;
    while (n_sightings < len &&
        read_sighting(f, &sightings[n_sightings]) == 1) {
...
int read_sighting(FILE *f, struct pod *s) {
    ...
    // finish string at '\n' if there is one
    // strchr will point to first occurence
